...
#
					...
					system("mv total.fits Save/$out_file_s[$i]");
				}
			}else{
...
#print "$first\n";
				for($i = 0; $i < 10; $i++){
					...
					system("chimgtyp out.fits  $fit_file datatype=LONG Inull=-99 clobber=yes");
					$mfile    = 'file'."$i";
...
#print "$file\n";
					...
					for($i = 0; $i < 10; $i++){
						...
						system("fimgmerge temp3.fits \@zfile mtemp.fits");
				...
				for($i = 0; $i < 10; $i++){
					$fit_file = 'total'."$i".'.fits';
					system("mv $fit_file Save/$out_file_s[$i]");
					system("gzip Save/$out_file_s[$i]");
				}
			}
		}
#
...
#rawx=6144:10239:1, rawy=6144:10239:1]
		...
		if($hrci_cnt > 0){
			if($hrci_cnt == 1){
				...
				system("mv $file Save/$out_file_i[$i]");
			}else{
				...
				for($i = 0; $i < 9; $i++){
					$fit_file = 'total'."$i".'.fits';
					system("mv $fit_file Save/$out_file_i[$i]");
					system("gzip  Save/$out_file_i[$i]");
				}
			}
		}
	}
